...
func (s *Snapshot) RangeCheck() error {
	if s.InstrumentId < s.InstrumentIdMinValue() || s.InstrumentId > s.InstrumentIdMaxValue() {
		return fmt.Errorf("%w on s.InstrumentId (%v < %v > %v)", ErrRangeCheck, s.InstrumentIdMinValue(), s.InstrumentId, s.InstrumentIdMaxValue())
	}

	if s.TimestampMs < s.TimestampMsMinValue() || s.TimestampMs > s.TimestampMsMaxValue() {
		return fmt.Errorf("%w on s.TimestampMs (%v < %v > %v)", ErrRangeCheck, s.TimestampMsMinValue(), s.TimestampMs, s.TimestampMsMaxValue())
	}

	if s.ChangeId < s.ChangeIdMinValue() || s.ChangeId > s.ChangeIdMaxValue() {
		return fmt.Errorf("%w on s.ChangeId (%v < %v > %v)", ErrRangeCheck, s.ChangeIdMinValue(), s.ChangeId, s.ChangeIdMaxValue())
	}
...
func (s *SnapshotLevelsList) RangeCheck() error {
	...
	if s.Price < s.PriceMinValue() || s.Price > s.PriceMaxValue() {
		return fmt.Errorf("%w on s.Price (%v < %v > %v)", ErrRangeCheck, s.PriceMinValue(), s.Price, s.PriceMaxValue())
	}

	if s.Amount < s.AmountMinValue() || s.Amount > s.AmountMaxValue() {
		return fmt.Errorf("%w on s.Amount (%v < %v > %v)", ErrRangeCheck, s.AmountMinValue(), s.Amount, s.AmountMaxValue())
	}
